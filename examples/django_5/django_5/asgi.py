...
session_manager_cm = session_manager.run()  # this is an async context manager
session_manager_context = None  # Will hold context manager's __aexit__ for shutdown

async def on_startup():
    logging.info("Starting up session manager or other services")
    global session_manager_context
    session_manager_context = await session_manager_cm.__aenter__()

async def on_shutdown():
    logging.info("Shutting down session manager or other services")
    global session_manager_context
    if session_manager_context:
        await session_manager_cm.__aexit__(None, None, None)

# # Define your startup and shutdown logic:
# async def on_startup():
#     logging.info("Starting up session manager or other services")
#     # e.g. await session_manager.run() or start background tasks
#
# async def on_shutdown():
#     logging.info("Shutting down session manager or other services")
